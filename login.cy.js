...
describe("Connexion utilisateur", () => {
  ...
  it("SC11 - devrait désactiver le bouton si les champs sont vides", () => {
    // 1.   Cliquer sur "SIGN IN" sans rien remplir
    cy.getBySel("signin-submit").click();

    // 2. Le message d'erreur "Username is required" apparaît sous le champ
    cy.get("#username-helper-text").should("be.visible").and("contain", "Username is required");

    // 3. Sans rien taper, vérifier que le bouton "SIGN IN" est disabled
    cy.getBySel("signin-submit").should("be.disabled");

    // 4. Taper seulement le username
    cy.getBySel("signin-username").type("Heath93");

    // 5. Vérifier que le bouton est toujours disabled
    cy.getBySel("signin-submit").should("be.disabled");

    // 6. Taper aussi le password
    cy.getBySel("signin-password").type("s3cret");

    // 7. Vérifier que le bouton est maintenant enabled
  ...
  it("SC12 - devrait afficher une erreur si le username est effacé après saisie", () => {
    ...
    cy.getBySel("signin-username").type("test");
